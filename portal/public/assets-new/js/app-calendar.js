...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function n(e) {
    return e.id ? "<div class='d-flex flex-wrap align-items-center'>" + e.text + '</div>' : e.text;
  }
  ...
  function s() {
    ...
    (e.setAttribute('data-bs-toggle', 'sidebar'),
      ...
      e.insertAdjacentHTML('beforeend', '<i class="ti ti-menu-2 ti-sm text-heading"></i>'));
  }
  (x.length &&
  ...
  let r = new Calendar(v, {
    ...
    dateClick: function (e) {
      ...
      (u(),
        C.show(),
        h && (h.innerHTML = 'Planifier une réunion'),
        (y.innerHTML = 'Ajouter'),
  ...
  e.config.onChange.push(function (e) {
    (r.changeView(r.view.type, moment(e[0]).format('YYYY-MM-DD')),
      s(),
      m.classList.remove('show'),
      f.classList.remove('show'));
  });
...
function checkForTimeClashes(newEvent, eventsArray) {
  ...
  for (const existingEvent of eventsArray) {
    ...
    const commonGuests = JSON.parse(newEvent.guests).filter((guest) =>
      existingEvent.extendedProps.guests.includes(guest)
    );
    ...
    if (commonGuests.length > 0) {
      ...
      if (
        (newEventStart.isSameOrAfter(existingEventStart) &&
          newEventStart.isBefore(existingEventEnd)) ||
