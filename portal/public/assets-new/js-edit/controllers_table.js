  ...
  $.TableEdit.plugin = {
    ...
    _addNewRow: function (o) {
      ...
      for (var col = 0; col < o.newRow.length; col++) {
        if (
          o.checkedRow !== undefined &&
          (o.checkedRow[col].mx == 3 || o.checkedRow[col].mx == 4)
        ) {
          ...
          o.newRow[col] = this.newCell(o.newRow[col], {
            group: o.group,
            rowIndex: o.shiftIndex,
            colIndex: col,
          });
        }
      }
      ...
      if (add !== undefined) {
        $(add).before(
          this.doMethod('_createRow', {
            tr: this.createEL('tr'),
            row: o.data[o.shiftIndex],
            index: o.shiftIndex,
            group: o.data,
            td: o.td,
          })
        ...
        this.getNodeGroup(o.group).appendChild(
          this.doMethod('_createRow', {
            tr: this.createEL('tr'),
            row: o.data[o.shiftIndex],
            index: o.shiftIndex,
    ...
    _correctCell: function (params) {
      ...
      var node = this.doMethod('_getFrontCell', {
        row: parent.rowIndex,
        col: parent.colIndex,
        group: params.group,
      });
      ...
      this.attr(node, params.property, +this.attr(node, params.property) + params.correct);
    },
    ...
    _shiftCell: function (params) {
      ...
      if (params.direction === 'rowspan') {
        ...
        data[params.rowIndex + 1][params.colIndex] = $.extend(
          true,
          {},
          data[params.rowIndex][params.colIndex]
        );
        movable = this.doMethod('_getFrontCell', {
          row: params.rowIndex,
          col: params.colIndex,
          group: params.group,
        });
        wanted = this.doMethod('_getFrontCell', {
          row: params.rowIndex + 1,
          col: params.colIndex + (+this.attr(movable, 'colspan') || 1),
          group: params.group,
        });
        if (wanted == undefined) {
      ...
      if (params.direction === 'colspan') {
        ...
        data[params.rowIndex][params.colIndex + 1] = $.extend(
          true,
          {},
          data[params.rowIndex][params.colIndex]
        );
        ...
        while (
          data[++rowIndex] &&
          data[rowIndex][params.colIndex] &&
          data[rowIndex][params.colIndex].mx == 3
        ) {
    ...
    _deleteRow: function (o) {
      ...
      for (var col = 0; col < o.pullOutRow.length; col++) {
        if (
          o.pullOutRow[col].hasOwnProperty('attr') &&
          o.pullOutRow[col].attr.hasOwnProperty('rowspan') &&
    ...
    _addNewColumn: function (o) {
      ...
      $(this.thead)
        .find('tr[data-controls]')
        .find('td')
        .eq(o.scene)
        [method](this.html(this.createEL('td'), this.topControlsElements));
      $(this.tfoot)
        .find('tr[data-controls]')
        .find('td')
        .eq(o.scene)
        [method](this.html(this.createEL('td'), this.bottomControlsElements));
      var row,
        length,
        groups = {
          thead: 'th',
      ...
      for (var group in groups) {
        ...
        if (
          o.part &&
          this.hasOwnProperty('maxRowsOutDelay') &&
          o.newCol.length > this.maxRowsOutDelay
        ) {
    ...
    addNewDelayedCols: function (o) {
      ...
      setTimeout(function generateCol() {
        var save = that.howCreateOnce * interation,
          length =
            o.newCol[o.group].length - save < that.howCreateOnce
              ? o.newCol[o.group].length - save
              : that.howCreateOnce;
    ...
    _addNewCol: function (o) {
      if (o.checkedCell !== undefined && (o.checkedCell.mx == 2 || o.checkedCell.mx == 4)) {
        if (
          o.newCol[o.group][o.rowIndex] === undefined ||
          o.newCol[o.group][o.rowIndex].mx === undefined
        ) {
        ...
        o.newCol[o.group][o.rowIndex] = this.newCell(o.newCol[o.group][o.rowIndex], {
          group: o.group,
          rowIndex: o.rowIndex,
          colIndex: o.scene,
        });
      ...
      var that = this;
      $(o.tr)
        .find('td[data-real-index],th[data-real-index]')
        .each(function () {
      ...
      if (d >= 0) {
        ...
        if (destination !== undefined) {
          $(destination).after(
            this.createCell(o.tr, o.data[o.rowIndex], cell, o.rowIndex, o.scene, o.data, o.td)
          );
        }
      }
      if (d == -1 && this.controlOrientation == 'right')
        $(o.tr).prepend(
          this.createCell(o.tr, o.data[o.rowIndex], cell, o.rowIndex, o.scene, o.data, o.td)
        );
    ...
    _deleteColumn: function (o) {
      ...
      for (var group in groups) {
        ...
        if (
          ...
          o.group === 'tbody' &&
          this.hasOwnProperty('maxRowsOutDelay') &&
          o.data.length > this.maxRowsOutDelay
        ) {
    ...
    deleteDelayedCols: function (o) {
      ...
      setTimeout(function delCol() {
        var save = that.howCreateOnce * interation,
          length =
            o.data.length - save < that.howCreateOnce ? o.data.length - save : that.howCreateOnce;
        for (var row = 0; row < length; row++) {
    ...
    _deleteCol: function (o) {
      var remove = true;
      if (
        o.checkedCell.hasOwnProperty('attr') &&
        o.checkedCell.attr.hasOwnProperty('colspan') &&
      ...
      o.deleted = o.data[o.rowIndex].splice(
        o.pullOutIndex,
        1
      ) /* get first coz count equal one */[0];
      if (remove)
        $(
          this.doMethod('_getFrontCell', { row: o.tr, col: o.pullOutIndex, group: o.group })
        ).remove();
      var that = this;
      $(o.tr)
        .find('td[data-real-index],th[data-real-index]')
    ...
    _getFrontRow: function (params) {
      ...
      if (typeof params.group === 'string') {
        if (this.provideGroup(params.group) === 'thead') {
          params.rowIndex += this.getNodeGroup(params.group).querySelectorAll(
            'tr[data-controls]'
          ).length;
        }
    ...
    _getFrontCell: function (params) {
      if (params.row instanceof jQuery) {
        return (params.td = params.row.find(
          'td[data-real-index="' + +params.col + '"],th[data-real-index="' + +params.col + '"]'
        )[0]);
      }

      if (params.row.nodeName) {
        return (params.td = params.row.querySelector(
          'td[data-real-index="' + +params.col + '"],th[data-real-index="' + +params.col + '"]'
        ));
      }

      return (params.td = this.doMethod('_getFrontRow', {
        rowIndex: params.row,
        group: params.group,
      }).querySelector(
        'td[data-real-index="' + +params.col + '"],th[data-real-index="' + +params.col + '"]'
    ...
    _change: function (params) {
      var countCol, countRow;

      if (params.newData.hasOwnProperty('attr')) {
        if (
          params.newData.attr.hasOwnProperty('colspan') &&
          params.getTune('colspan', params.newData) < params.getTune('colspan', params.cell)
        ) {
          countCol =
            params.getTune('colspan', params.cell) - params.getTune('colspan', params.newData);
          ...
          for (var row = 0; row < countRow; row++) {
            for (var col = 0; col < countCol; col++) {
              params.adding.push({
                ...
                cell: params.data[params.rowIndex + row][
                  params.colIndex + params.getTune('colspan', params.cell) - 1 - col
                ],
              });
            }
          }
        ...
        if (
          params.newData.attr.hasOwnProperty('rowspan') &&
          params.getTune('rowspan', params.newData) < params.getTune('rowspan', params.cell)
        ) {
          countCol =
            params.getTune('colspan', params.cell) - params.getTune('colspan', params.newData) > 0
              ? params.getTune('colspan', params.newData)
              : params.getTune('colspan', params.cell);
          countRow =
            params.getTune('rowspan', params.cell) - params.getTune('rowspan', params.newData);
          for (var row = 0; row < countRow; row++) {
            for (var col = 0; col < countCol; col++) {
              params.adding.push({
                group: params.group,
                ...
                cell: params.data[
                  params.rowIndex + params.getTune('rowspan', params.cell) - 1 - row
                ][params.colIndex + col],
              });
            }
          }
        ...
        if (
          ...
          countCol =
            params.getTune('colspan', params.newData) - params.getTune('colspan', params.cell);
          countRow =
            params.getTune('rowspan', params.cell) - params.getTune('rowspan', params.newData) > 0
              ? params.getTune('rowspan', params.newData)
              : params.getTune('rowspan', params.cell);
          for (var row = 0; row < countRow; row++) {
            for (var col = 0; col < countCol; col++) {
              var checkCell =
                params.data[params.rowIndex + row][
                  params.colIndex + params.getTune('colspan', params.cell) + col
                ];
              params.remove.push({
                group: params.group,
                rowIndex: params.rowIndex + row,
        ...
        if (
          ...
          countCol = params.newData.attr.colspan
            ? params.getTune('colspan', params.newData)
            : params.getTune('colspan', params.cell);
          countRow =
            params.getTune('rowspan', params.newData) - params.getTune('rowspan', params.cell);
          for (var row = 0; row < countRow; row++) {
            for (var col = 0; col < countCol; col++) {
              var checkCell = params.data[
                params.rowIndex + params.getTune('rowspan', params.cell) + row
              ]
                ? params.data[params.rowIndex + params.getTune('rowspan', params.cell) + row][
                    params.colIndex + col
                  ]
                : undefined;
              params.remove.push({
    ...
    _handleContraction: function (params) {
      if (params.stretchError.rowspan.length == 0 && params.stretchError.colspan.length == 0) {
        for (var i = 0; i < params.adding.length; i++) {
          ...
          var $tr = $(
            this.doMethod('_getFrontRow', { rowIndex: el.rowIndex, group: params.group })
          );
          do {
            if (params.data[el.rowIndex][col] === undefined) {
              if (this.controlOrientation === 'left') {
                $tr.append(
                  this.createCell(
                    $tr[0],
                    params.data[el.rowIndex],
                    el.cell,
                    el.rowIndex,
                    el.colIndex,
                    params.data,
                    this.provideGroup(params.group) === 'tbody' ? 'td' : 'th'
                  )
                );
                break;
              }
