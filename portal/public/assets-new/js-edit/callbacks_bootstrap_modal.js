jQuery(document).ready(function ($) {
  ...
  $.TableEdit.callbacks = {
    eventsBindAfter: function () {
      $('#TableEditModal').on('show.bs.modal', this, function (e) {
        var that = e.data,
          $form = $(this).find('form'),
        ...
        $form
          ...
          .attr('data-row', row)
          .attr('data-col', col)
          .attr('data-uniq', uniqueID);

        $form.find('#value').val(data[row][col].val);

        var colspan =
          (data[row][col].attr && data[row][col].attr.colspan && data[row][col].attr.colspan) || 1;
        var rowspan =
          (data[row][col].attr && data[row][col].attr.rowspan && data[row][col].attr.rowspan) || 1;
        $form.find('#colspan').val(colspan).attr('max', that.getNumOfCols());
        $form.find('#rowspan').val(rowspan).attr('max', data.length);
      });

      $('.save-cell-changes').on('click', this, function (e) {
        var that = e.data,
