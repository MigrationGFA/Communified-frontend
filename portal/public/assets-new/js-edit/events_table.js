jQuery(document).ready(function ($) {
  ...
  $.TableEdit.plugin = {
    _eventsBind: function () {
      ...
      if (!this.uniqueID)
        this.uniqueID =
          this.getNumOfCols() + this.getGroup('B').length + Math.floor(Math.random() * 900 + 100);

      $thead.on('click._addRow', '.addrow', this, function (e) {
        e.data.addNewRows({
      ...
      $tbody.on('click._addRow', '.addrow', this, function (e) {
        e.data.addNewRows({
          scene: $(this).closest('tr').index(),
      ...
      $tfoot.on('click._addRow', '.addrow', this, function (e) {
        e.data.addNewRows({
          scene: $(this).closest('tr').index(),
      ...
      $thead.on('click._delRow', '.delrow', this, function (e) {
      ...
      $tbody.on('click._delRow', '.delrow', this, function (e) {
        if (confirm('Are you sure you want to delete?')) {
          e.data.deleteSomeRows({
            scene: $(this).closest('tr').index(),
            group: 'B', // coz event for tbody
      ...
      $tfoot.on('click._delRow', '.delrow', this, function (e) {
        if (confirm('Are you sure you want to delete')) {
          e.data.deleteSomeRows({
            scene: $(this).closest('tr').index(),
            group: 'F', // coz event for tfoot
      ...
      $thead.on('click._addCol', '.addCol', this, this.addCol);

      $tfoot.on('click._addCol', '.addCol', this, this.addCol);

      $thead.on('click._delCol', '.delCol', this, this.delCol);

      $tfoot.on('click._delCol', '.delCol', this, this.delCol);
      ...
      $('body').on('click._editCell contextmenu._editCell', this, function (e) {
      ...
      $table.on('cell:editing:start', this.cellEditingStart);

      $table.on('cell:editing:stop', this.cellEditingStop);
    },
    ...
    editingStart: function (e, extra) {
      ...
      if (
        that.cache &&
        that.cache.editableCell &&
        that.cache.isEditCell &&
        that.cache.editableCell.is($this)
      )
        return;
    ...
    cellEditingStart: function (event, object) {
      ...
      var that = object.that,
        params = {
          ...
          $menuContent: $(
            '' +
              '<div class="edit-cell edit-cell-content" data-group="' +
              object.group +
              '" data-row="' +
              rowIndex +
              '" data-col="' +
              object.target.attr('data-real-index') +
              '" data-uniq="' +
              that.uniqueID +
              '">' +
    ...
    _cellEditingStart: function (params) {
      ...
      params.$target
        .html(params.$targetContent.css(params.$targetCss))
        .addClass('edit-cell')
        .find(params.$targetContent)
        .focus(function () {
