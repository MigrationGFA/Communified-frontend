...
jQuery(document).ready(function ($) {
  ...
  $.TableEdit.plugin = {
     ...
     */
    topControlsElements:
      '<a class="addCol" href="javascript://" role="button"><span class="glyphicon glyphicon-plus"></span></a>' +
     ...
     */
    rowControlsElements:
      '<a class="addrow" href="javascript://" role="button"><span class="glyphicon glyphicon-plus"></span></a>' +
    ...
    _addStub: function (params) {
      if (this.controlOrientation === 'right') {
        params.tr.appendChild(this.html(this.createEL('td'), this.stubElements));
      } else if (this.controlOrientation === 'left') {
        $(params.tr).prepend(this.html(this.createEL('td'), this.stubElements));
      }
    },
    ...
    _createTopControls: function (params) {
      for (var i = 0; i < this.getNumOfCols(); i++) {
        params.tr.appendChild(this.html(this.createEL('td'), this.topControlsElements));
      }
    ...
    _createBottomControls: function (params) {
      for (var i = 0; i < this.getNumOfCols(); i++) {
        params.tr.appendChild(this.html(this.createEL('td'), this.bottomControlsElements));
      }
      this.doMethod('_addStub', params);
    ...
    _createRowControls: function (params) {
      if (this.controlOrientation === 'right') {
        params.tr.appendChild(this.html(this.createEL('td'), this.rowControlsElements));
      } else if (this.controlOrientation === 'left') {
        $(params.tr).prepend(this.html(this.createEL('td'), this.rowControlsElements));
      }
      return params.tr;
    },
    ...
    _addTable: function (params) {
      $(this.doMethod('_defineOutputContainer', { selector: params.selector }))[
        this.doMethod('_defineOutputMethod', {})
      ](this.table);
    },
    ...
    _createCell: function (params) {
      ...
      if (params.col.val) this.html(params.td, params.col.val);

      if (params.col.callbacks) this.doMethod('_doDelayedFunction', params);
    ...
    _setMatrix: function (object) {
      if (object.col.mx && object.row.length == this._numberOfColumns)
    ...
    createDelayedRows: function (td) {
      ...
      setTimeout(function generateRows() {
        var save = context.howCreateOnce * interation,
          length =
            context.getGroup('B').length - save < context.howCreateOnce
              ? context.getGroup('B').length - save
              : context.howCreateOnce;
        for (var row = 0; row < length; row++) {
          context.tbody.appendChild(
            context.doMethod('_createRow', {
    ...
    _setDelayedFunction: function (destination, fn) {
      ...
      if (
        (this[name + 'Before'] &&
          ...
          this[name + 'Before'](params) == true) ||
        !this[name + 'Before']
      ) {
        if (
          params.destination.hasOwnProperty('callbacks') &&
          Array.isArray(params.destination.callbacks) &&
          typeof params.fn == 'function'
        ) {
    ...
    _createPage: function () {
      if (
        ...
        this.getGroup('B').length > this.maxRowsOutDelay
      ) {
