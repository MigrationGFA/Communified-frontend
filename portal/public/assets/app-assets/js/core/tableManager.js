...
(function ($) {
  ...
  $.fn.tablemanager = function (options = null) {
    ...
    var numOfPages =
      options.numOfPages !== undefined && options.numOfPages > 0 ? options.numOfPages : 5;
    ...
    if (options !== null) {
      ...
      if (
        ...
        for (var i = 0; i < options.disableFilterBy.length; i++) {
          // check if should be disabled last column
          col =
            options.disableFilterBy[i] == -1 || options.disableFilterBy[i] == 'last'
              ? Heads.length
    ...
    function multipleSortCol(array, data) {
      ...
      array.sort(function (a, b) {
        for (i = 0; i < cols.length; i++) {
          ...
          let firstCol = new DOMParser().parseFromString(a[cols[i]], 'text/html').documentElement
            .textContent;
          let secondCol = new DOMParser().parseFromString(b[cols[i]], 'text/html').documentElement
            .textContent;
