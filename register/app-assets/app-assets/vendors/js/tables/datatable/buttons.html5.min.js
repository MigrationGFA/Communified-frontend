  ...
  function E(a, b) {
    y === w &&
      (y =
        -1 ===
        C.serializeToString(
          new g.DOMParser().parseFromString(F['xl/worksheets/sheet1.xml'], 'text/xml')
        ).indexOf('xmlns:r'));
  ...
  v.ext.buttons.excelHtml5 = {
    ...
    action: function (a, b, d, c) {
                ...
                : ((m = m.replace ? m.replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, '') : m),
                  (g = r(h, 'c', {
                    ...
                    children: {
                      row: r(h, 'is', {
                        children: {
                          row: r(h, 't', { text: m, attr: { 'xml:space': 'preserve' } }),
                        },
                      }),
                    },
                  }))));
      ...
      for (y = k.header.length; x < y; x++)
        q.appendChild(
          r(h, 'col', { attr: { min: x + 1, max: x + 1, width: L(k, x), customWidth: 1 } })
        );
      q = a.xl['workbook.xml'];
      f('sheets sheet', q).attr('name', G(c));
      c.autoFilter &&
        (f('mergeCells', h).before(
          r(h, 'autoFilter', { attr: { ref: 'A' + d + ':' + A(k.header.length - 1) + t } })
        ),
